...
class Carousel extends Component {
  ...
  getDataIndex = (index) => {
    ...
    if (index >= dataLength + 1) {
      return dataLength < 1 ? (index - 1) % dataLength : index - dataLength - 1;
    }
    if (index < 1) {
  ...
  renderItem = ({item, index}) => {
    ...
    return (
      <View pointerEvents="box-none" {...specificProps}>
        {renderItem({item, index})}
      </View>
    );
  };
  ...
  getComponentStaticProps = () => {
    ...
    const containerStyle = [{width: sliderWidth, flexDirection: 'row'}];
