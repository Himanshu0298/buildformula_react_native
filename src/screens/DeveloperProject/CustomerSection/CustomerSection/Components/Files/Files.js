...
function Files(props) {
  ...
  console.log('-------->index_of', folderDepth);

  const ScrollViewContainer = folderDepth ? ScrollView : Tabs.ScrollView;

  // TODO:Unable permission

  // const modulePermissions = getPermissions('Files');

  const {loading, versionData} = useSelector(s => s.customer);
  ...
  const [folder, setFolder] = React.useState(!!hideFolder);
  ...
  const loadData = () => {
    getFolder({project_id, unitid: unit.unit_id, index_of: folderDepth});
    getFile({project_id, unitid: unit.unit_id, folder_id: folderDepth});
  };
  ...
  const loadFolderData = () =>
    getFolder({project_id, index_of: folderDepth, unitid: unit.unit_id});
  const loadFileData = () =>
    getFile({project_id, unitid: unit.unit_id, folder_id: folderDepth});
