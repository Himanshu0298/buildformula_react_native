...
function RenderFolder(props) {
  ...
  const onPressFolder = () => {
    navigation.push('CustomerFiles', {
      ...route?.params,
      folder_name,
      index_of: folder.index_of + 1,
      hideFolder: true,
    });
  };
  ...
  return (
    <View style={styles.sectionContainer}>
      <TouchableOpacity style={{flexGrow: 1}} onPress={onPressFolder}>
        <View style={styles.sectionContainer}>
...
function FolderSection(props) {
  ...
  const {folders} = useSelector(s => s.customer);

  const filteredFolders = folders?.[folderDepth] || [];
  console.log('-------->folderDepth', folderDepth);
  console.log('-------->filteredFolders', filteredFolders.length);
