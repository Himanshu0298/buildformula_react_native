...
function BungalowsList(props) {
  ...
  const {fdBungalowsList, versionData, loading} = useSelector(
    s => s.designModule,
  );
  const {selectedProject} = useSelector(s => s.project);
  const project_id = selectedProject.id;

  const bungalowsList = fdBungalowsList?.data?.bungalows;
  ...
  const loadFiles = () => {
    getFDBungalows({project_id, folder_id: folderId, bunglow_plot_id: 0});
  };
