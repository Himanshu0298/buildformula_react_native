...
import VersionDialog from '../../FinalDrawing/Components/VersionDialog';
...
function WDTowerList(props) {
  ...
  const loadFiles = () => {
    getWDTowers({project_id, folder_id: folderId});
  };

  const renameFileHandler = async (name, id) => {
    await renameWDTowerFile({
      file_name: name,
      working_drawing_tower_file_id: id,
      project_id,
    });
    loadFiles();
    toggleDialog();
  ...
  const deleteFileHandler = async id => {
    await deleteWDTowerFile({working_drawing_tower_file_id: id, project_id});
    loadFiles();
    toggleDialog();
  ...
  const handleFileUpload = async file => {
    ...
    await uploadWDTowersFile(formData);
    toggleDialog();
    snackbar.showMessage({
  ...
  const onSelectStructure = (itemId, index) => {
    ...
    navigation.navigate('WDTowerPreview', {
      tower_id,
      folderId,
