...
function FloorDestinationFiles(props) {
  const {route, navigation} = props;
  const {folder_title, item: folder, folderId} = route?.params || {};

  const floorId = folder?.working_drawing_floor_rows_id;
  ...
  const {wdFolderFilesList, versionData, loading, wdFolderFileVersion} =
    useSelector(s => s.designModule);

  const project_id = selectedProject.id;
  const data = wdFolderFilesList?.final_drawing_tower_floors_files || [];
  ...
  const loadFiles = () => {
    getWDFloorFolderFile({
      ...
      working_drawing_floor_rows_folder_id: folder_id,
    });
  };
...
export default FloorDestinationFiles;
