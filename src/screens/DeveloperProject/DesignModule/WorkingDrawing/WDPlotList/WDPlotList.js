...
function WDPlotList(props) {
  ...
  const plotsList = plots?.plots || [];

  const plotFiles = plots?.working_drawing_bunglow_files;
  ...
  const loadFiles = () => {
    getWDplots({project_id, folder_id: folderId});
  };
  ...
  const handleFileUpload = async file => {
    ...
    await uploadWDPlotsFile(formData);
    toggleDialog();
    snackbar.showMessage({
  ...
  const onSelectStructure = values => {
    ...
    navigation.navigate('WDPlotFileList', {
      tower_id,
      folderId,
      towerLabel,
      structureLabel,
